...
abstract class JSON_WebServer_Controller extends CI_Controller
{
    ...
    private function ProcessCall()
    {
        ...
        if (! isset($input_params['method'])) {
            $this->ReturnError('No se ha indicado el método que se debe invocar. Envíe en ' . 'el POST, JSON con el formato ("method":XXX, "arguments":XXX)');
        }
        $method_name = $input_params['method'];
        if (! method_exists($this, $method_name)) {
            $this->ReturnError('No está declarado el método <' . $method_name . '> en el servicio');
        }
        
        // Han indicado los argumentos de la función
        if (! isset($input_params['arguments'])) {
            $this->ReturnError('No se ha indicado los argumentos del método. Envíe en ' . 'el POST, JSON con el formato ("method":XXX, "arguments":XXX)');
        } else 
            if (! is_array($input_params['arguments'])) {
        ...
        if ($return_value === NULL) {
            ...
            $this->ReturnError("Error en la llamada al metodo [$method_name] " . "con los parámetros [$str_args]");
        }
    ...
    /**
     * Documenta las funciones disponibles.
    ...
    {
        $this->documentacionFunciones[] = array(
            'signature' => $signature,
            'help' => $help
        );
    }
    ...
    /**
     * Devuelve como resultado error en la operación solicitada.
     * Se indica
     * el motivo del error.
     *
     ...
     * 
     * @param mixed $value
     *            Valor devuelto, cualquier tipo convertible en JSON
     */
...
<body>
	...
	<p>Enviar parametros en formato JSON mediante POST utilizando libreria
		JSON_WebClient</p>
</body>
</html>
<?php
